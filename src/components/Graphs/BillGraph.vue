<template>
    <v-card elevation="1" class="tw-min-w-[300px]">
        <div class="tw-flex tw-flex-col tw-items-center tw-justify-center tw-p-2">
            <apexchart type="radialBar" :options="chartOptions" :series="scaledSeries" />
            <table class="tw-w-full">
                <tbody>
                    <tr v-for="item in series_data" :key="item.name" class="tw-h-10 tw-w-full">
                        <td class="tw-text-sm tw-font-semibold tw-px-2">
                            <v-chip class="tw-text-sm tw-font-semibold" :color="item.color" :outlined="true"
                                :small="true">
                                {{ item.name }}
                            </v-chip>
                        </td>
